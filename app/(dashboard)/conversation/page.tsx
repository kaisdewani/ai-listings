    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        ...
        try {
            const systemMessage: ChatCompletionRequestMessage = {
                role: "system",
                content: "You are a real estate agent who is specialized in creating listings for homes.(250 word limit)"
            };
            ...
            const newMessages = [...messages, systemMessage, userMessage1, userMessage2];

            const response = await axios.post("/api/conversation", {
                messages: newMessages,
            });

            setMessages((current) => [...current, userMessage1, userMessage2, response.data]);
    ...
    return (
        ...
        "
                    >
                        ...
                        <div className="col-span-12 lg:col-span-10 p-4 rounded-lg border w-full mt-4">
                            <FormField
                                name="keywords"
                                render={({ field }) => (
                                    <FormControl className="m-0 p-0">
